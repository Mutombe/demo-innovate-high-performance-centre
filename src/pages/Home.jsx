    ...
    <section ref={containerRef} className="relative h-screen min-h-[700px] overflow-hidden bg-black">
      <motion.div className="absolute inset-0" style={{ y: bgY }}>
        <AnimatePresence mode="sync">
          <motion.img key={currentSlide} src={heroImages[currentSlide]}
            alt="Innovate High Performance Centre" className="absolute inset-0 w-full h-[130%] object-cover object-center"
      ...
      <motion.div className="relative z-20 flex flex-col items-center justify-center h-full text-center max-w-5xl mx-auto px-5 sm:px-8 pt-36"
        ...
        <div className="overflow-hidden">
          {(hero.titleLines || ['NO LIMITS', 'NO', 'EXCUSES.']).map((line, i) => (
...
function StatsSection() {
  ...
  const defaults = [{ number: '287', label: 'Reviews' }, { number: '4.3', label: 'Stars' }, { number: '800', label: 'Members Transformed' }, { number: '50', label: 'Classes Weekly' }];
...
function CTASection() {
  ...
  return (
    <section ref={ref} className="relative py-28 sm:py-36 lg:py-48 overflow-hidden">
      <motion.div className="absolute inset-0" style={{ y: bgY }}>
        <img src="https://images.unsplash.com/photo-1571019614242-c5c5dee9f50b?w=1200&q=80" alt="Innovate High Performance Centre" className="w-full h-[130%] object-cover object-center" loading="eager" />
      ...
      <div className="relative z-20 max-w-5xl mx-auto px-5 sm:px-8 lg:px-12 text-center">
        <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 1 }}>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            <a href={`https://wa.me/${business.whatsappNumber || '263772225292'}`} target="_blank" rel="noopener noreferrer"
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <motion.div initial={{ opacity: 0, x: -30 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.8 }}>
            ...
            <div className="space-y-5">
              {[{ icon: MapPin, text: business.address || 'The Chase, Harare, Zimbabwe' }, { icon: Phone, text: business.phone || '+263 77 222 5292' }, { icon: WhatsappLogo, text: 'Message on WhatsApp' }].map((item, i) => (
          ...
          <motion.div initial={{ opacity: 0, x: 30 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.8, delay: 0.15 }} className="aspect-[4/3] overflow-hidden">
            <img src="https://images.unsplash.com/photo-1517836357463-d25dfeac3438?w=1200&q=80" alt="Innovate High Performance Centre" className="w-full h-full object-cover object-center" loading="eager" />
          </motion.div>
        </div>
      </div>
    </section>
